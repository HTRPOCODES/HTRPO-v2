...
class RobotSuiteWrapper(object):
    ...
    def step(self, action):
        ...
        done = (self.n_steps >= self.max_episode_steps) or reached_goal
...
class SawyerPickPlace(RobotSuiteWrapper):
    def __init__(self,
        ...
        self.bin_tgt = (
            (bin_pos[0] - bin_size[0] / 4, bin_pos[1] - bin_size[1] / 4, bin_pos[2] + 0.1),
            (bin_pos[0] + bin_size[0] / 4, bin_pos[1] - bin_size[1] / 4, bin_pos[2] + 0.1),
            (bin_pos[0] - bin_size[0] / 4, bin_pos[1] + bin_size[1] / 4, bin_pos[2] + 0.1),
            (bin_pos[0] + bin_size[0] / 4, bin_pos[1] + bin_size[1] / 4, bin_pos[2] + 0.1),
        )
    ...
    def read_desired_goal(self):
        if self.n_steps == 0:
            random_xy = 0.3 * np.random.rand(2) - 0.15
            random_z = 0.2 * np.random.rand(1)
            random_perturb = np.concatenate((random_xy, random_z))
            if not self.all_obj:
                self.tgt = np.array(self.bin_tgt[self.object_id]) + random_perturb
            else:
                self.tgt = (np.array(self.bin_tgt) + random_perturb).reshape(-1)
        return self.tgt
